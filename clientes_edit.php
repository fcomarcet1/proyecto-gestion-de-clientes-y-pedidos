...
<?php
if ((isset($_REQUEST["idCliente"])) && (isset($_REQUEST["intProvincia"]))) {

  $intProvincia = $_REQUEST["intProvincia"];
          ...
          <form action="clientes_update.php" name="form1" id="form1" method="post">
            ...
            <div class="form-group">
              ...
              <select class="custom-select mr-sm-2" name="provincia">
                <option value="provincia">Selecciona Provincia</option>
                <?php
                while ($rowprovincia = mysqli_fetch_array($query)) {
                  if ($rowprovincia["idProvincia"] == $intProvincia) {
                    echo "<option value='" . $rowselectId["idProvincia"] . "' selected='selected'>" . $rowprovincia['strNombreProvincia'] . "</option>";
                  } else {
